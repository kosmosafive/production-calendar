...
- **Автоматическое управление памятью**: встроенный механизм очистки кэша предотвращает утечки памяти при работе с большими диапазонами дат (по умолчанию хранится не более 5 лет).
...
use Kosmosafive\ProductionCalendar\Provider\JsonFileProvider;
...
Класс `ProductionCalendar` автоматически управляет внутренним кэшем данных о праздниках:

- По умолчанию хранится информация максимум за **5 лет**.
- При запросе данных для нового года старые записи автоматически удаляются.
- Это предотвращает утечки памяти при длительной работе приложения или обработке больших диапазонов дат.

При необходимости можно изменить лимит, отредактировав константу `MAX_CACHE_YEARS` в классе `ProductionCalendar`.
